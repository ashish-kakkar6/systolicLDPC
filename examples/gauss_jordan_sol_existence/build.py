...
def main() -> None:
    ...
    run_cycles = int(args.run_cycles if args.run_cycles is not None else ((2 * n_cols) + m_rows + l_cols - 1))
    src_depth = next_src_depth(m_rows)
    has_solution_sw = gf2_has_solution_columns(a_mat, b_mat)
    ...
    tb_path = paths["generated"] / "tb_example_gauss_jordan_sol_existence.sv"
    tb_path.write_text(
        render_tb(
            tb_template,
            n=n_cols,
    ...
    meta = {
        ...
        "files": {
            ...
            "tb_sv": str((final_case_dir / "generated" / "tb_example_gauss_jordan_sol_existence.sv").resolve()),
        },
    }
    save_meta(temp_case_dir / "meta.json", meta)

    manifest = {
        "workflow": "gauss_jordan_sol_existence",
        "case_id": case_id,
        ...
        "files": {
            ...
            "tb_sv": "generated/tb_example_gauss_jordan_sol_existence.sv",
        },
        "hashes": {
    ...
    print_case_report(
        final_case_dir,
        "stage: build",
        f"M={m_rows} N={n_cols} L={l_cols} run_cycles={run_cycles}",
